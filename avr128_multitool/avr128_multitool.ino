...
float latituderec[4];
float longituderec[4];
uint8_t coordrectptr;
...
#define poc_pttpin PIN_PC0
float poc_freq = 150.2083;
float poc_dev = 1.5;
uint32_t poc_addr = 705234;
//uint32_t addr = 0400001;
#define rate1200period 845
uint16_t poc_rateperiod[3] = {rate1200period * 2, rate1200period, rate1200period / 2}; //1200
uint8_t poc_rate = 1; //0 - 512, 1 - 1200, 2 - 2400
bool poc_invert;
#define poc_msglength 64
char poc_msg[poc_msglength] = {'c', 'o', 'r', 'p'};
uint8_t poc_msgptr = 4;
bool poc_mode; //0 - direct, 1 - rfm95
bool poc_enablertransmit_;
bool poc_startrtransmit;
    ...
    if(!enter) { //prevents from adding a character before sending
      switch(menuselect) {
        ...
        case rfm95chat__: //rfm95 chat
          ...
          break;
        case pager__:
          terminaloutput(poc_msg, sizeof(poc_msg), serialread, poc_msgptr); //write to text field
          break;
      }
    }
...
//            delay(1);
    ...
    case si5351vfo__:
      ...
      break;
    case pager__:
      if(!poc_keybenable) { //while keyboard is not open
        if(sidepanelenable) { //only while sidepanel is open
          if(b0) {
            menuselect = apps__; //go to apps
            sidepanelenable = 0; //close sidepanel
...
//                    pager_init();
            ...
            case 5: //deviation
              if(blockmenunavigation) {
                ...
                if(globalbuttons) {
                  rfm95.setFrequencyDeviation(poc_dev);
                }
              }
              break;
            case 6: //tune period
              if(blockmenunavigation) {
                ...
                if(globalbuttons) {
                  poc_rateperiod[0] = poc_rateperiod[1] * 2;
                  poc_rateperiod[2] = poc_rateperiod[1] / 2;
                }
              }
              break;
            case 7: //send
              if(enter) {
                transmitpocsag(poc_mode);
              }
              break;
      ...
      } else { //while keyboard is open
        keyboardinterface(poc_msg, sizeof(terminalfieldbuffer), poc_msgptr);
        if(keybrowselect == 4) { //only last row
          if(enter) {
            switch(keybcolselect) {
              case 0: //caps
                ...
                break;
              case 1: //space
                addspace(poc_msg, poc_msgptr);
                break;
              case 2: //exit
                poc_keybenable = 0; //exit out to terminal
                blockmenunavigation = 0; //remove lock
                break;
              case 3: //send
                transmitpocsag(poc_mode);
                break;
              case 4: //clear
                cleardatabuffer(poc_msg, sizeof(poc_msg), poc_msgptr);
                break;
              case 5: //backspace
                removelastchar(poc_msg, poc_msgptr);
                break;
            }
          }
        }
...
//        rfm95recvscreen();
      ...
      case si5351vfo__: //si5351 vfo
        ...
        break;
      case pager__:
        if(!poc_keybenable) { //while keyboard is not opened
          u8g2.setCursor(0, 8); u8g2.print("addr: "); u8g2.print(poc_addr);
          u8g2.setCursor(0, 8*2); u8g2.print("freq: "); u8g2.print(poc_freq, 5); u8g2.print("MHz");
          uint16_t rate_number = 0;
          switch(poc_rate) {
            case 0: rate_number = 512; break;
            case 1: rate_number = 1200; break;
            case 2: rate_number = 2400; break;
          }
          u8g2.setCursor(0, 8*3); u8g2.print("rate: "); u8g2.print(rate_number);
          u8g2.setCursor(0, 8*4); u8g2.print("invert: "); u8g2.print(poc_invert ? "yes" : "no");
          u8g2.setCursor(0, 8*5); u8g2.print("mode: "); u8g2.print(poc_mode ? "rfm95" : "direct");
          u8g2.setCursor(0, 8*6); u8g2.print("dev: "); u8g2.print(poc_dev); u8g2.print("KHz");
//          u8g2.setCursor(0, 8*7); u8g2.print("msg: "); u8g2.print(poc_msg);

          u8g2.setCursor(0, 8*7); u8g2.print("tune period: "); u8g2.print(poc_rateperiod[1]); u8g2.print("us");
          u8g2.setCursor(0, 8*8); u8g2.print(">>>send");
...
void gpsmapscreen() {
  ...
  if(lastgpsxpos != gpsxpos || lastgpsypos != gpsypos) { //check for changes
    ...
    for(uint8_t i = 1; i < 4; i++) { //move values down
      latituderec[i-1] = latituderec[i];
      longituderec[i-1] = longituderec[i];
    }

    latituderec[3] = latitude;
    longituderec[3] = longitude;
  }
  lastgpsxpos = gpsxpos;
  lastgpsypos = gpsypos;
...
//  gpsxpos = 63;
  ...
  float degrees = calc_degrees(longituderec[0], latituderec[0], longitude, latitude); //using raw coordinates for better precision
...
void transmitpocsag(bool mode) {
  pocsag.CreatePocsag(poc_addr,0,poc_msg,0,1);
  if(mode) {
    rfm95.transmitDirect();
    delay(100);
    data2bitseq((uint8_t *)pocsag.GetMsgPointer(), pocsag.GetSize(), poc_rateperiod[poc_rate]);
    delay(100);
    rfm95.standby();
  } else {
    pinMode(poc_pttpin, OUTPUT);
    digitalWrite(poc_pttpin, 0); //turn pa on
    delay(200);
    data2bitseq((uint8_t *)pocsag.GetMsgPointer(), pocsag.GetSize(), poc_rateperiod[poc_rate]);
    delay(100);
    pinMode(poc_pttpin, INPUT);
  }
}

bool data2bitseq(uint8_t* data, size_t len, uint16_t udelay) {
  for(uint16_t byteselect = 0; byteselect < len; byteselect++) {
    for(byte i = 8; i > 0; i--) {
      pinwrite(poc_fskportbit, bitread(data[byteselect], i-1));
      delayMicroseconds(udelay);
    }
  }
  pinwrite(poc_fskportbit, 0); //make final state low
}

bool bitread(byte value, byte select) {
  return (value >> select) & 0b00000001;
}

void pinwrite(uint8_t bitselect, bool bit_) {
  if(poc_invert) {bit_ = !bit_;}
  if(bit_) {
    poc_fskport.OUTSET = 0b00000001 << bitselect; //bit set
  } else {
    poc_fskport.OUTCLR = 0b00000001 << bitselect; //bit clear
  }
//  digitalWrite(poc_fskpin, bit_);
}
